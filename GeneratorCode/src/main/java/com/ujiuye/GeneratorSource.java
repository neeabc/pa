...
public class GeneratorSource {
	...
	private static String path_2 = "xiao";
...
//			String src = IOUtils.toString(new FileInputStream(".\\src\\main\\resources\\template\\[Table2]Service.java"), "utf-8");
		...
		if (srcJava != null && !"".equals(srcJava)) {
			// 创建目录保存源码
			File f1 = new File(
					"GeneratorCode/src/main/java/" + path_1 + "/" + path_2 + "/" +modelName+ "/service/");
			if (!f1.exists()) {
				f1.mkdirs();
			}
			// 创建源码
...
//					new FileInputStream(".\\src\\main\\resources\\template\\[Table2]ServiceImpl.java"), "utf-8");
		...
		if (srcJava != null && !"".equals(srcJava)) {
			// 创建目录保存源码
			File f1 = new File("GeneratorCode/src/main/java/" + path_1 + "/" + path_2 + "/" +modelName
					+ "/service/impl/");
			if (!f1.exists()) {
				f1.mkdirs();
			}
...
//					.toString(new FileInputStream(".\\src\\main\\resources\\template\\[table2]Controller.js"), "utf-8");
		...
		if (srcJava != null && !"".equals(srcJava)) {
			// 创建目录保存源码
			File f1 = new File("GeneratorCode/src/main/webapp/js/controller/");
			if (!f1.exists()) {
				f1.mkdirs();
			}
			// 创建源码
...
//					"utf-8");
		...
		if (srcJava != null && !"".equals(srcJava)) {
			// 创建目录保存源码
			File f1 = new File("GeneratorCode/src/main/webapp/js/service/");
			if (!f1.exists()) {
				f1.mkdirs();
			}
			// 创建源码
...
//					"utf-8");
		...
		if (srcJava != null && !"".equals(srcJava)) {
			// 创建目录保存源码
			File f1 = new File("GeneratorCode/src/main/webapp/admin/");
			if (!f1.exists()) {
				f1.mkdirs();
			}
			// 创建源码
